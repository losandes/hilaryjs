...
For Node, hilary supports auto-registration of your modules, if you export an object with ``name``, ``dependencies``, and/or ``factory`` properties. If you define a ``name``, the module will be registered. If you omit the ``name``, it will be resolved (immediately executed).

```
module.exports.name = 'myModule';
module.exports.dependencies = ['myOtherModule'];
modue.exports.factory = function (myOtherModule) {
    // module code here
};
```
...
```
hilary.autoRegister(require('./myModule'));
```

You can use ``autoRegister`` with objects and arrays, to support Node indexes. Suppose you have a controllers folder with an index file the exports the following:
...
Then you can just ``autoRegister`` the index:

```
hilary.autoRegister('./controllers');
```
